...
FROM ct_CIDADES CIDADE, ct_ESTADO ESTADO
...
SELECT CIDADE.Nome as CIDADE, ESTADO.Nome as ESTADO
FROM ct_CIDADES CIDADE, ct_ESTADO ESTADO
WHERE idEstado = ESTADO.id and ESTADO.Nome in ('São Paulo', 'Rio de Janeiro')
...
FROM ct_CIDADES CIDADE
INNER JOIN ct_ESTADO on ct_ESTADO.id = CIDADE.idEstado
...
SELECT *
FROM  ct_CIDADES
WHERE idEstado in (SELECT id FROM ct_ESTADO WHERE Nome = 'São Paulo')
